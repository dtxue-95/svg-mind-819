import type { MindMapData, MindMapNodeData, RawNode } from '../types';
...
export const createInitialMindMap = (rawData: RawNode): MindMapData => {
    ...
    const traverse = (rawNode: RawNode, parentUuid: string | null) => {
        ...
        if (rawNode.nodeType === 'caseNode') {
            ...
            if (rawNode.functionTestCaseDTO) tags.push('function');
            if (rawNode.apiTestCaseDTO) tags.push('api');
            if (rawNode.uiTestCaseDTO) tags.push('ui');
